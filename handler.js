...
export async function handler(chatUpdate) {
    ...
    try {
       ...
       if (opts['gconly'] && !m.chat.endsWith('g.us') && !m.fromMe)
            return conn.reply(m.chat, '*BOT MODE GCONLY*\nlink gc klo mau masuk:https://chat.whatsapp.com/HUSKMik8Lck90NinohIbre\n\nMAU ```SEWA BOT CHAT OWNER👇 Wa.me//6285759681568```',m)
...
export async function participantsUpdate({ id, participants, action }) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/a9e1822c24c549a1e8749.jpg'
                    try {
 ...
 let godbye = 'https://telegra.ph/file/deaf59bc3e5216eaae814.jpg' 
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? pp : pp)).buffer(), '제노 봇이라는 화분한 봇이 있습니다.', text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'hehe🗿',
     )
...
const fgclink = {
  ...
  if (rown) return conn.sendButtonDocAccess(m.chat, `${ucapan()} ` + tag, rown + '\n\n\n\n', 'Owner', '.owner', m, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'https://chat.whatsapp.com/HUSKMik8Lck90NinohIbre',
    mediaType: 2, 
    description: sgc,
